        ...
        for msg, exchange_section, needed_methods in params:
            with self.subTest(
                ...
                for ccxtMethod in needed_methods:
                    if (
                        ccxtMethod not in self.exchange.has
                        or not self.exchange.has[ccxtMethod]
                    ):  # ccxt unified method check
                        if not hasattr(
                            self.exchange, ccxtMethod
                        ):  # ccxt implicit (not unified) method check
                            self.fail(
                                msg=f"{ccxtMethod}() not available for exchange {self.exchange.name}"
                            )
