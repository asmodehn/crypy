...
from crypy.desk.impl.mpmath import (
    MPFloat,
    MPFuzzyFloat,
    MPInterval,
    MPBoundedFloat,
    MPBoundedFloatException,
    MPBoundedFuzzyFloat,
    MPBoundedFuzzyFloatException,
    isfinite,
)
...
def underbounds(draw,):
    ...
    v = draw(
        MPFloat.strategy(max_value=float(b.a), exclude_max=True, allow_infinity=False)
    )
    return v, b
...
def overbounds(draw,):
    ...
    v = draw(
        MPFloat.strategy(min_value=float(b.b), exclude_min=True, allow_infinity=False)
    )
    return v, b
